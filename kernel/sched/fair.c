		   ...
		   struct cfs_rq *cfs_rq)
{
	 ...
	 */
	if (!accumulate_sum(delta, cpu, sa, load, runnable, running, cfs_rq))
		return 0;
...
___update_load_avg(struct sched_avg *sa, unsigned long load, unsigned long runnable)
{
	...
	sa->loadwop_avg = div_u64(sa->loadwop_sum, divider);
...
{
	if (___update_load_sum(now, cpu, &se->avg, 0, 0, 0, NULL)) {
...
{
	if (___update_load_sum(now, cpu, &rt_rq->avg, running, running,
			       running, NULL)) {
